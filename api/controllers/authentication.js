  ...
  try {
    ...
    if (!req.body?.email || !req.body?.password) {
      logger.warn(
        ...
        "authentication",
        "login",
        `${!req.body?.email ? "email is undefined" : "password is undefined"}`,
        {}
      );
    ...
    logger.info(
      ...
      "authentication",
      "login",
      "Fetching user data :: Calling getUserByEmail()",
      {}
    );
    ...
    if (user.isLocked === "true" || user.isSuspended === "true") {
      logger.warn(
        ...
        "authentication",
        "login",
        `${user.isLocked ? "account is locked" : "account is suspended"}`,
        { email: req.body.email }
      );
      return res.status(403).json(commonResponses.invalidLoginCredentials);
    }
    ...
    logger.info(
      ...
      "authentication",
      "login",
      "Starting MySQL Transaction :: Calling beginTransaction()",
      {}
    );
    ...
    if (!bcrypt.compareSync(req.body.password, user.password)) {
      ...
      const increment = await User.incrementPreviousLockCountAttempts(
        connection,
        user.ID,
        req.body.requestID
      );
      ...
      if (!increment) {
        logger.error(
          ...
          "authentication",
          "login",
          "Failed to increment failed login attempts :: Rolling Back MySQL Transaction :: Calling rollback()",
          { userID: user.ID }
        );
      ...
      logger.info(
        ...
        "authentication",
        "login",
        "Committing MySQL Transaction :: Calling commit()",
        {}
      );
    ...
    if (user.previousLockCount !== 0) {
      logger.info(
        ...
        "authentication",
        "login",
        "Resetting previous lock count :: Calling resetPreviousLockCount()",
        {}
      );
      if (
        !(await User.resetPreviousLockCount(
          connection,
          user.ID,
          req.body.requestID
        ))
    ...
    logger.info(
      ...
      "authentication",
      "login",
      "Calling generateAccessToken() :: Calling generateRefreshToken() :: In Promise.all()",
      {}
    );
    ...
    if (!promisesData.accessToken || !promisesData.refreshToken) {
      logger.error(
        ...
        "login",
        `Failed to create ${
          promiseData.accessToken ? "refresh" : "access"
        } token :: Rolling Back MySQL Transaction :: Calling rollback()`,
        {}
      );
      await connection.rollback();
...
exports.logout = async (req, res) => {
  try {
    logger.info(
      ...
      "logout",
      "Starting execution",
      { ipAddress: req.ip, userID: req.body.userData.ID }
    );
    ...
    if (
      !promisesData.blacklistAccessToken ||
      !promisesData.blacklistRefreshToken
    ) {
      logger.error(
        req.body.requestID,
        "authentication",
        "logout",
        `Failed to blacklist ${
          !promisesData.blacklistAccessToken ? "access" : "refresh"
        } token :: 
                Rolling Back MySQL Transaction :: Calling rollback()`,
        {}
      );
...
exports.changePassword = async (req, res) => {
  try {
    ...
    if (
      !(await require("../../helpers/password-validator").isPasswordValid(
    ...
    logger.info(
      ...
      "authentication",
      "changePassword",
      "Calling isCurrentPasswordMatches() :: Calling changeUserPassword() :: In Promise.all()",
      { userID: req.body.userData.ID }
    );
    const promises = await Promise.all([
      User.isCurrentPasswordMatches(
        connection,
    ...
    if (
      !promisesData.isCurrentPasswordMatches ||
      !promisesData.changeUserPassword
    ) {
      logger.error(
        req.body.requestID,
        "authentication",
        ...
        `${
          !promisesData.isCurrentPasswordMatches
            ...
            : "Failed to change user password"
             ...
             Rolling Back MySQL Query :: Calling rollback()`,
        { userID: req.body.userData.ID }
      );
      await connection.rollback();
  ...
  try {
    ...
    if (!user) {
      logger.info(
        ...
        "authentication",
        "requestResetPassword",
        "Email does not exists or inactive",
        { email: req.query.email }
      );
    ...
    logger.info(
      ...
      "authentication",
      "requestResetPassword",
      "Creating reset pin :: Calling createResetPasswordPin()",
      {}
    );
    const resetPasswordPin = await Pin.createResetPasswordPin(
      connection,
      user.ID,
      req.query.requestID
    );
...
exports.confirmResetPassword = async (req, res) => {
  try {
    ...
    logger.info(
      ...
      "authentication",
      "confirmResetPassword",
      "Checking if the new password is valid via password-validator package",
      {}
    );
    if (
      !(await require("../../helpers/password-validator").isPasswordValid(
    ...
    logger.info(
      ...
      "authentication",
      "confirmResetPassword",
      "Checking reset password pin if valid :: Calling isResetPasswordPinValid()",
      { pin: req.body.pin }
    );
    const user = await Pin.isResetPasswordPinValid(
      connection,
      req.body.pin,
      req.body.requestID
    );
    ...
    if (!user) {
      logger.warn(
        ...
        "authentication",
        "confirmResetPassword",
        "Reset password pin is not valid or does not exists",
        { pin: req.body.pin }
      );
    ...
    if (
      !promisesData.changeUserPassword ||
      !promisesData.destroyResetPasswordPin
    ) {
      logger.error(
        req.body.requestID,
        "authentication",
        ...
        `${
          !promisesData.changeUserPassword
            ? "Failed to change user password"
            : "Failed to destroy reset password pin"
        } 
            :: Rolling Back MySQL Query :: Calling rollback()`,
