...
exports.verifyRefreshToken = async (token) => {
    try {
        ...
        if (error.name === 'TokenExpiredError') {
            logger.warn(new Date().getTime(), 'json-web-token', 'verifyRefreshToken', 'Token has expired', {});
            return false;
        } else {
...
exports.verifyAccessToken = async (token) => {
    try {
        ...
        if (error.name === 'TokenExpiredError') {
            logger.warn(new Date().getTime(), 'json-web-token', 'verifyAccessToken', 'Token has expired', {});
            return false;
        } else {
            throw new Error(error);
        }
